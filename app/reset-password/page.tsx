...
export default function ResetPasswordForm({ email }: { email: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!hasValidEmail) {
      setErrorMessage(
        "Missing or invalid email information. Please go back and request a new OTP."
      )
      return
    }
            ...
            <div className="mt-8 grid gap-4">
              <div className="rounded-2xl border border-primary/10 bg-background/80 p-4 shadow-sm">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Your reset session is already tied to the email address used in
                  the previous step.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-primary/10 bg-background/80 p-4 shadow-sm">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Once your OTP is confirmed, your new password will replace your
                  old one immediately.
                </p>
              </div>
            </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 truncate text-sm font-medium text-foreground">
                          {hasValidEmail ? email : "Email not available"}
                        </p>
                      </div>
                    </div>
                  </div>
